...
namespace Fahrrad_ERP
{
    public partial class Bestellung_wählen : Form
    {
        ...
        {
            string sqlcmd = "SELECT BestellungID, kunden.Firma, kunden.Nachname, kunden.Name, Datum, Bestellsumme FROM bestellung JOIN kunden ON bestellung.BestellungID = kunden.KundenID";
            Database_Fahrrad daten = new Database_Fahrrad();
            dataList = daten.getData(sqlcmd);
            listView1.Items.Clear();
            foreach (List<string> list in dataList)
            {
                listView1.Items.Add(new ListViewItem(new string[] { list[0].ToString(), list[1].ToString() + list[2].ToString() +", "+ list[3].ToString(), (Convert.ToDateTime(list[4])).ToString("d"), (Convert.ToDecimal(list[5])).ToString("0.00 €") }));
            }
            set_nummer(0);
        }
